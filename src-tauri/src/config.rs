...
pub struct ReaderConfig {
    ...
    pub image_gap: u32,
    #[serde(rename = "blankPagePosition")]
    pub blank_page_position: String,
}
...
impl ConfigManager {
    ...
    pub fn get_current_api_domain(&self) -> Result<String, String> {
            ...
            || config.current_api_index as usize >= config.api_sources.len()
        {
            ...
            // 保存更新后的配置
            if let Err(e) =
                self.save_app_config(config.api_sources.clone(), config.current_api_index)
            {
    ...
    pub fn save_app_config(
        ...
        // 读取现有配置以保留轻小说源配置
        let existing_config: Result<AppConfig, String> =
            self.path_helper.read_config(ConfigFiles::APP);
