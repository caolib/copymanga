...
pub async fn download_cartoon_chapter(
    ...
    match download_video(&client, &download_info.video_url, &video_path).await {
        Ok(file_size) => {
...
async fn download_video(
    ...
    if url.ends_with(".m3u8") {
        return download_hls_stream(client, url, save_path).await;
    }
...
#[tauri::command]
pub async fn check_cartoon_chapter_downloaded(
    cartoon_uuid: String,
    chapter_uuid: String,
    app_handle: AppHandle,
) -> Result<CheckDownloadedResult, String> {
...
pub async fn get_downloaded_cartoon_list(
    ...
    let download_dir = resource_dir.join("downloads").join("anime");
