...
async fn proxy_handler(
    ...
    // 检查是否是外部URL参数格式 (/proxy?url=external_url)
    if path.is_empty() && query.starts_with("?url=") {
        // 提取URL参数值（简单的字符串处理）
        let url_param = &query[5..]; // 去掉 "?url=" 前缀
        
        // 简单的URL解码（只处理%编码）
        let external_url = url_param.replace("%3A", ":").replace("%2F", "/").replace("%3F", "?").replace("%3D", "=").replace("%26", "&");
        ...
        for (k, v) in resp.headers().iter() {
            if !["content-length", "transfer-encoding", "connection"].contains(&k.as_str()) {
