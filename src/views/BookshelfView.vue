      ...
      <a-tabs v-model:activeKey="activeTab" @change="handleTabChange" type="card" >
        <template #rightExtra>
          <div class="tab-extra-actions">
            ...
            <a-typography-text
              type="secondary"
              v-if="getCurrentTabUpdateTime() && !loading"
              class="update-time"
            >
        ...
        <a-tab-pane key="manga" tab="漫画收藏">
          ...
          <MangaCollection
            :loading="loading && activeTab === 'manga'"
            @update-count="handleMangaCountUpdate"
            @update-time="(time) => handleUpdateTime('manga', time)"
            ref="mangaCollectionRef"
          />
        </a-tab-pane>
        <!-- 轻小说收藏标签页 -->
        <a-tab-pane key="book" tab="轻小说收藏">
          ...
          <BookCollection
            :loading="loading && activeTab === 'book'"
            @update-count="handleBookCountUpdate"
            @update-time="(time) => handleUpdateTime('book', time)"
        ...
        <a-tab-pane key="cartoon" tab="动画收藏">
          ...
          <CartoonCollection
            :loading="loading && activeTab === 'cartoon'"
            @update-count="handleCartoonCountUpdate"
            @update-time="(time) => handleUpdateTime('cartoon', time)"
        ...
        <a-tab-pane key="post" tab="写真收藏">
          ...
          <PostCollection
            :loading="loading && activeTab === 'post'"
            @update-count="handlePostCountUpdate"
            @update-time="(time) => handleUpdateTime('post', time)"
            ref="postCollectionRef"
          />
        </a-tab-pane>
      </a-tabs>
    </div>
