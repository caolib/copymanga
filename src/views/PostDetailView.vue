          ...
          <div v-else-if="chapters.length > 0" class="chapters-list">
            <a-list :data-source="chapters" item-layout="horizontal">
              <template #renderItem="{ item }">
                <a-list-item>
                  <template #actions>
                    <a-button type="primary" @click="readChapter(item)"> 阅读 </a-button>
                  </template>
                  <a-list-item-meta>
                    <template #title>
...
const toggleCollect = async (isCollect) => {
  ...
  await collectPost(postId, isCollect)
    .then(() => {
...
const goBack = () => {
  ...
  if (from === 'topic') {
    ...
    router.push({
      name: 'TopicDetail',
      params: { topicId: router.currentRoute.value.query.topicId || '' },
    })
  } else if (from === 'reader') {
    // 从阅读页返回时，去写真首页
