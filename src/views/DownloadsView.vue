                            ...
                            <div v-else class="task-list">
                                <a-list :data-source="taskList" item-layout="horizontal">
                                    <template #renderItem="{ item }">
                                        <a-list-item
                                            :key="`${item.id}-${item.status}-${Math.floor(item.progress / 5) * 5}`">
                                            <template #actions>
                                                <a-button v-if="item.status === 'downloading'" size="small"
                                                    @click="pauseTask(item)">
                                                    暂停
                                                </a-button>
                                                <a-button v-if="item.status === 'paused'" size="small" type="primary"
                                                    @click="resumeTask(item)">
                                                    继续
                                                </a-button>
                                                <a-button v-if="item.status !== 'downloading'" size="small" danger
                                                    ...
                                                    <div class="task-description">
                                                        <div class="task-progress">
                                                            ...
                                                            <div class="progress-info">
                                                                <span class="progress-text">{{ item.progressText ||
                                                                    '准备下载...' }}</span>
...
// 任务统计计算属性
const activeTaskCount = computed(() => taskList.value.filter(task => task.status === 'downloading').length)
const pausedTaskCount = computed(() => taskList.value.filter(task => task.status === 'paused').length)

// 页面生命周期
onMounted(() => {
    // 先初始化下载管理器，然后加载任务列表
    cartoonDownloadManager.initialize().then(() => {
        loadTaskList()
...
const loadTaskList = async () => {
    try {
        ...
        // 只有当任务列表发生实际变化时才更新
        // 使用更精确的比较，避免不必要的重新渲染
        const hasChanged = taskList.value.length !== newTaskList.length ||
