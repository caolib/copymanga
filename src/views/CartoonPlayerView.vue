    ...
    <a-card :bordered="false" class="player-card">
      <a-skeleton :loading="loading" active>
        ...
        <div class="video-info" v-if="videoData.name">
          <a-typography-title
            :level="3"
            style="cursor: pointer; color: #1890ff; width: fit-content"
          ...
          <div class="chapters-list" v-if="cartoonPlayerStore.currentCartoonChapters.length">
            ...
            <div class="chapters-grid">
              <a-button
                v-for="chapter in cartoonPlayerStore.currentCartoonChapters"
                :key="chapter.uuid"
                :type="chapter.uuid === route.params.chapterId ? 'primary' : 'default'"
                size="small"
          ...
          <div class="line-selector" v-if="videoData.lines">
            <div class="line-controls">
              <div class="line-selection">
                ...
                <a-radio-group
                  v-model:value="currentLine"
                  @change="onLineChange"
                  style="margin-left: 12px"
                >
                  <a-radio-button
                    v-for="(line, key) in videoData.lines"
                    :key="key"
                    :value="key"
              ...
              <div class="local-video-section" style="margin-top: 12px">
                <a-button
                  v-if="isLocalVideoAvailable"
                  type="primary"
                  :icon="h(FolderOpenOutlined)"
                  @click="openLocalVideoDirectoryHandler"
                  size="small"
                >
                  打开本地视频目录
                </a-button>
...
const fetchVideoData = (line) => {
  ...
  getVideoByChapterId(pathWord, chapterId, selectedLine)
    .then(async (response) => {
...
const initializePlayer = async () => {
  ...
  dp.value.on('loadedmetadata', () => {
    ...
    // 将按钮插入到控制栏
    const controllerBar = document.querySelector(
