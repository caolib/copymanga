<template>
  <a-card class="manga-detail-container" :bordered="false">
      ...
      <a-row :gutter="32">
        ...
        <a-col :xs="24" :sm="16">
          ...
          <a-descriptions :column="1" size="small" bordered>
            ...
            <a-descriptions-item
              label="最新章节"
              v-if="manga.last_chapter && manga.last_chapter.name"
            >
          ...
          <div style="margin-top: 20px">
            ...
            <div v-if="fromCollection && lastBrowseInfo" class="last-browse-info">
              <a-alert
                :message="`上次阅读: ${lastBrowseInfo.last_browse_name || '未知章节'}`"
                type="info"
                show-icon
                style="margin-bottom: 16px"
                :description="
                  lastBrowseInfo.datetime_browse
                    ? `阅读时间: ${formatDate(lastBrowseInfo.datetime_browse)}`
                    : null
                "
              />
            </div>
    ...
    <a-row justify="space-between" align="middle" style="margin-bottom: 12px">
      <a-col>
        <a-space>
          ...
          <!-- 来自书架时显示继续阅读按钮 -->
          <a-button
            v-if="fromCollection && lastBrowseInfo && lastBrowseInfo.last_browse_id"
            type="primary"
          ...
          <a-button @click="fetchMangaData" :loading="detailLoading" size="small"
            >刷新数据</a-button
          >
        </a-space>
      </a-col>
    ...
    <!-- 分组Tab页 -->
    <a-tabs
      v-model:activeKey="currentGroup"
    ...
    <a-skeleton :loading="loading || groupLoading" active>
      <a-row :gutter="[12, 12]">
        ...
        >
          <a-card
            :hoverable="true"
            style="text-align: center; padding: 0; position: relative"
            size="small"
          ...
          >
            <!-- 下载状态标签 - 左上角 -->
            <a-tag
              v-if="chapterDownloadStatus[chapter.id] === 'downloaded'"
              color="success"
            ...
            <a-tag
              v-else-if="chapterDownloadStatus[chapter.id] === 'downloading'"
              color="processing"
              class="download-status-tag"
            >
              <template #icon>
            ...
            <div @click="goToChapter(chapter)" style="cursor: pointer; margin-bottom: 8px">
              ...
              <!-- 显示"上次阅读"标记 -->
              <div v-if="isLastReadChapter(chapter)" class="last-read-tag">上次</div>
            </div>
            ...
            <div class="chapter-actions" style="width: 100%">
              <!-- 下载进度条 (仅在下载中时显示) -->
              <div
                v-if="chapterDownloadStatus[chapter.id] === 'downloading'"
                style="width: 100%; margin-bottom: 8px"
              >
                <a-progress
                  :percent="chapterDownloadProgress[chapter.id] || 0"
              ...
              <div style="display: flex; justify-content: center; align-items: center; gap: 4px">
                ...
                <a-button
                  v-if="
                    !chapterDownloadStatus[chapter.id] ||
                    chapterDownloadStatus[chapter.id] === 'error'
                  "
                  size="small"
                  type="primary"
    ...
    <!-- 漫画评论区 -->
    <a-collapse
      v-model:activeKey="commentsActiveKey"
    ...
    >
      <a-collapse-panel key="comments" header="漫画评论">
        <!-- 评论输入框 -->
        <div
          v-if="commentsActiveKey.includes('comments')"
