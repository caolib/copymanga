...
export function useMangaDetail() {
  ...
  const sortedChapters = computed(() => {
    return [...chapters.value].sort((a, b) =>
      isAscending.value ? a.index - b.index : b.index - a.index,
    )
  })
  ...
  const startReading = () => {
    if (chapters.value.length > 0) {
      const firstChapter = isAscending.value
        ? chapters.value[0]
        : chapters.value[chapters.value.length - 1]
  ...
  const handleCollect = (isCollect = true) => {
    ...
    collectManga(manga.value.uuid, isCollect)
      .then(() => {
  ...
  const submitComment = async () => {
    ...
    await postMangaComment(manga.value.uuid, commentText)
      .then(() => {
  ...
  const downloadChapter = async (chapter, forceRedownload = false) => {
    ...
    try {
      ...
      // 调用下载API
      return downloadChapterAPI(route.params.pathWord, chapter.id, chapterInfo, (progressInfo) => {
        // 更新进度
        ...
        .then(() => {
          ...
          // 重新检查章节的真实下载状态
          checkChapterDownloadStatus([chapter])
            .then(() => {
              // 根据检查结果显示不同的消息
  ...
  const downloadCurrentPage = async () => {
    const chaptersToDownload = chapters.value.filter(
      (chapter) => chapterDownloadStatus.value[chapter.id] !== 'downloaded',
    )
  ...
  const deleteChapter = async (chapter) => {
    ...
    await downloadManager
      .deleteChapter(manga.value.uuid, groupPathWord, chapter.uuid)
      .then(() => {
        // 更新下载状态
